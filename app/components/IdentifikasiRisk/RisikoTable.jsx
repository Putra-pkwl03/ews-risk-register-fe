...
export default function RisikoTable({
  ...
  // Saat pertama kali komponen mounting
  useEffect(() => {
  ...
  // Setiap kali ada perubahan pada addedRiskIds, simpan ke localStorage
  useEffect(() => {
            ...
            >
              ...
              <td className="p-2 text-xs max-w-[250px]">
                {item.causes?.map((cause) => (
                  <div key={cause.id} className="mb-2">
                    ...
                    >
                      <strong>Sub :</strong>{" "}
                      {cause.sub_causes?.map((sub) => sub.sub_cause).join(", ")}
                    </p>
                  </div>
                ))}
              </td>
              ...
              <td className="p-2 text-sm m:p-3">
                <div className="flex flex-row justify-center items-center gap-2">
                  ...
                  <button
                    onClick={() => {
                      if (addedRiskIds?.includes(item.id)) return;
                    ...
                    title={
                      addedRiskIds?.includes(item.id)
                        ? "Risiko ini sudah ditambahkan"
                        : "Tambah Risiko"
                    }
                    disabled={addedRiskIds?.includes(item.id)}
