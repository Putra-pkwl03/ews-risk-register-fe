...
export default function EditMitigationForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Semua mitigasi berhasil diperbarui.");
      router.push("/dashboard?page=evaluasi-risiko");
    } catch (err) {
      console.error("Gagal update mitigasi:", err);
      alert(err.message || "Gagal memperbarui mitigasi.");
    }
  };
  ...
  return (
    <>
      ...
      <h2 className="text-xl font-semibold mb-2">Edit Mitigasi</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 ">
          {forms.map((form, mitIndex) => (
            <div
              key={form.id}
              className="p-4 border rounded shadow-md bg-white space-y-4 h-full min-w-[300px] flex flex-col justify-between"
            >
              <div className="space-y-2">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleChange(mitIndex, "mitigation_type", e.target.value)
                    }
