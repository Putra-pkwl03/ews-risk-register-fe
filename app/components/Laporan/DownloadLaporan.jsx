...
const DownloadLaporan = () => {
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(12);
    doc.text("Risk Handling Report", 14, 15);
    ...
    doc.save(`risk_handling_report_${rangeType || "all"}.pdf`);
  };
  ...
  const exportToExcel = () => {
    ...
    const rows = getFilteredData().map((item, index) => [
      ...
      item.reviewer?.name || "-",
      new Date(item.created_at).toLocaleDateString(),
      item.approval_signature ? "✅ Signed" : "-",
    ]);

    const ws = XLSX.utils.aoa_to_sheet([headers, ...rows]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Risk Handling Report");
    ...
    saveAs(blob, `risk_handling_report_${rangeType || "all"}.xlsx`);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 text-black overflow-x-auto">
      <div className="flex flex-wrap gap-4 justify-between items-start sm:items-center mb-6">
        <h1 className="text-2xl font-bold whitespace-nowrap">Risk Report</h1>

        <div ref={wrapperRef} className="relative inline-block text-left">
          <button
            onClick={() =>
              exportType === "open" ? setExportType(null) : setExportType("open")
            }
      ...
      <table className="w-full text-sm text-left overflow-auto max-h-[75vh] rounded">
        <thead className="bg-gray-100 text-[#5932EA] text-left border-b">
          <tr>
            ...
            <th className="px-2 py-1">Risk Name</th>
            ...
            <th className="px-2 py-1">Effectiveness</th>
        ...
        <tbody>
          {data.map((item, idx) => (
            ...
            >
              ...
              <td className="px-2 py-1">{item.risk?.monitoring_plan}</td>
              ...
              <td className="px-2 py-1 text-center">
                <img src={item.approval_signature} alt="Signature" className="w-20 h-auto mx-auto" />
              </td>
            </tr>
          ))}
        </tbody>
